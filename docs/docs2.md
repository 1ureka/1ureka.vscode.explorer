...
# `@view/action/contextmenu.ts`

提供右鍵選單的註冊與關閉邏輯：

- `registerContextMenu()` — 於 `window` 上註冊 `contextmenu` 事件（capture 階段），右鍵時阻止預設行為、記錄錨點座標至 `appStateStore.contextMenuAnchor`，並嘗試解析點擊位置對應的資料列索引。若有文字選取（`Range`）則不觸發。目前索引相關的 context menu 內容仍為 TODO。
...
使用 CSS Grid 以 `1fr auto 0.35fr 0.3fr` 佈局排列四個操作群組。
...
| Store | 型別 | 說明 |
|-------|------|------|
| `appStateStore` | `AppState` | 應用程式 UI 狀態（左側面板、右鍵選單錨點） |
...
- `readInitData()` — 初始化函式：
  1. 並行請求 `system.read.user.paths`（系統資料夾）與 `system.read.volumes`（磁碟機列表），結果存入 `navigationExternalStore`
